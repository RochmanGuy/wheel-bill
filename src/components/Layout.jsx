...
export default function Layout({ children }) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-100">
      ...
      {/* Content */}
      <main className="flex-1 overflow-auto pb-24" id="main-content">
        {children}
      </main>

      {/* Bottom Nav */}
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-lg z-50" aria-label="ניווט ראשי">
        <div className="flex">
          {NAV_ITEMS.map(item => {
        ...
        <div className="text-center py-1 border-t border-gray-100">
          <Link to="/accessibility" className="text-xs text-gray-400 underline focus:ring-2 focus:ring-blue-500 focus:outline-none rounded">
            הצהרת נגישות
          </Link>
        </div>
      </nav>
